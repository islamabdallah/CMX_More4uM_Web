      ...
      new Country() { Id = 2, Name = "Cairo" }
        ...
        public async Task<ActionResult> Index(BenefitFilterModel filterModel)
        {
            ...
            try
            {
               ...
               // benefitFilterModel = (BenefitFilterModel)null;
                ...
                return (ActionResult)reportController.View((object)benefitFilterModel);
            }
            catch (Exception ex)
            {
        ...
        public async Task<ActionResult> ExportExcel(BenefitFilterModel filterModel)
        {
            ...
            try
            {
                ...
                {
                    MemoryStream excel = _BenefitService.ExportBenefitsToExcel(filterModel.BenefitRequests);
        ...
        {
            List<Benefit> listAsync = await _repository.Find(i => i.IsVisible == true, false, b => b.BenefitType).Where(t => t.Country == id).ToListAsync();
            List<BenefitModel> benefitModelList = new List<BenefitModel>();
            benefitModelList = _mapper.Map<List<BenefitModel>>(listAsync);
           // Func<Task<List<BenefitModel>>> func = new Func<Task<List<BenefitModel>>>(this._BenefitService.GetAllBenefits);
            return JsonSerializer.Serialize(benefitModelList);
        }
